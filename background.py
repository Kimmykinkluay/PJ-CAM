...
if not imgModeList:
    print("No mode images loaded. Please check the image directory.")

# Loading the encoding file
try:
    with open('Enddoegenerator.py', 'rb') as file:
        encodeListKnownWithIds = pickle.load(file)
    encodeListKnown, studentIds = encodeListKnownWithIds
...
while True:
    ...
    for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
        ...
        if len(faceDis) > 0:  # Check if faceDis is not empty
        ...
        if counter != 0:
            if counter == 1 and id != -1:
                studentInfo = db.reference(f'face_images/{id}').get()
                print(studentInfo)
